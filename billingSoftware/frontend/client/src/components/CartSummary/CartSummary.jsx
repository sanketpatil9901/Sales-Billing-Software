...
const CartSummary = ({
  ...
  const completePayment = async (paymentMode) => {
    ...
    try {
      ...
      if (response.status === 201 && paymentMode === "cash") {
        ...
        if (!razorpayLoaded) {
          toast.error("Unable to laod razorpay");
          await deleteOrderOnFailure(savedData);
          return;
        }
        ...
        const options = {
          ...
          handler: async function (response) {
            await veriftPaymentHandler(response), savedData;
          },
          prefill: {
            name: customerName,
  ...
  const veriftPaymentHandler = async (response, savedOrder) => {
    const paymentData = {
      razorpayOrderId: response.razorpay_order_id,
      razorpatPaymentId: response.razorpay_payment_id,
        ...
        showPopup && (
            <ReceiptPopup
                orderDetails={{
                    ...
                    razorpatPaymentId: orderDetails.paymentDetails?.razorpatPaymentId,
                }}
